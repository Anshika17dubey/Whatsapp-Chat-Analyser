...
def fetch_stats(selected_user, df):
    if selected_user == "Overall":
        ...
        new_df =  df[df['user'] == selected_user]  # Count messages by selected user
        num_messages = new_df.shape[0]
        words = []
        for message in new_df['msg']:
            words.extend(message.split())
        num_media_msg = df[df['msg'] == '<Media omitted>'].shape[0]
        links = []
        for messages in df['msg']:
...
def emoji_helper(selected_user,df):
    ...
    emojis = []
    for message in df['msg']:
        emojis.extend([c for c in message if emoji.is_emoji(c)])  # Use emojis.extend()
